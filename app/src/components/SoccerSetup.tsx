...
const TEAM_COPY = {
  coral: {
    name: "Coral Comets",
    callout: "Fast breaks",
    description: "Warm, fearless, and built to press high up the pitch.",
  },
  teal: {
    name: "Teal Tides",
    callout: "Clean control",
    description: "Cool under pressure with sharp passing through the middle.",
  },
    ...
    <main className={`soccer-setup soccer-setup--${team}`}>
      ...
      <section className="soccer-setup__stage" aria-labelledby="soccer-setup-heading">
        <div className="soccer-setup__copy">
          <p className="soccer-setup__eyebrow">Three minutes. One wild finish.</p>
          <h1 id="soccer-setup-heading">Pick your side.</h1>
          <p>Lead a toy-block squad through quick passes, brave tackles, and one enormous final whistle.</p>
      ...
      <section className="soccer-setup__console" aria-label="Match setup">
        <fieldset className="soccer-setup__teams">
          ...
          {(Object.keys(TEAM_COPY) as SoccerTeamId[]).map((teamId) => {
            ...
            return (
              ...
              >
                <span className="soccer-team__crest" aria-hidden="true"><i /><b>{teamId === "coral" ? "C" : "T"}</b></span>
          ...
          <fieldset>
            ...
            <div className="soccer-choice-row">
              <button type="button" data-format="quick" aria-pressed={format === "quick"} onClick={() => setFormat("quick")}>
                <strong>Quick Play</strong><span>Solo + AI</span>
              </button>
              <button type="button" data-format="crew" aria-pressed={format === "crew"} onClick={() => setFormat("crew")}>
                <strong>Crew Match</strong><span>Multiplayer</span>
              </button>
            </div>
          </fieldset>
          <fieldset>
        ...
        <aside className="soccer-setup__launch">
          ...
          <p>Move with WASD or the touch stick. Pass or shoot with Space.</p>
        </aside>
      </section>
    </main>
  );
  ...
  return (
    <div className="soccer-stadium" aria-hidden="true">
      <svg viewBox="0 0 720 430" role="presentation">
        ...
        <g className="soccer-stadium__player soccer-stadium__player--teal" transform="translate(472 175)"><ellipse cy="23" rx="28" ry="11" fill="#3b0855" opacity=".25"/><path d="m-17-20 34 0 8 44-50 0z" fill="#30c0b7" stroke="#174b62" strokeWidth="6"/><circle cy="-37" r="17" fill="#9b674e" stroke="#174b62" strokeWidth="6"/><path d="m-9 24-5 29m27-29 5 29" stroke="#174b62" strokeWidth="10" strokeLinecap="round"/></g>
      ...
      <p><span>{team === "coral" ? "Home" : "Away"}</span> Meadowbank Arena</p>
    </div>
  );
}
